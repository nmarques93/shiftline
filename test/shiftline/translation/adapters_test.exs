defmodule Shiftline.Translation.AdaptersTest do
  @moduledoc """
  ...
  alias Shiftline.Translation.{Claude, DeepSeek, Prompt}
  ...
  describe "the suite is insulated from the network" do
    test "the configured adapter is the offline stub, whatever keys are in the shell" do
      ...
      assert Shiftline.Translation.adapter() == Shiftline.Translation.Stub
    end

    test "no API key reaches the test environment" do
      refute Application.get_env(:shiftline, Shiftline.Translation, [])[:api_key]
    end
