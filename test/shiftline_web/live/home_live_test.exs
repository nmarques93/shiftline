defmodule ShiftlineWeb.HomeLiveTest do
  @moduledoc """
  ...
  describe "who can do what" do
    ...
    test "Approve is offered for a real offer and withheld from a decline", %{
      ...
      theo = Shiftline.Repo.get_by!(Shiftline.Coordination.StaffMember, name: "Theo Martin")
  ...
  describe "the coverage loop through the UI" do
    test "respond, approve, then acknowledge", %{conn: conn, request: request, luis: luis} do
      ...
      assert Shiftline.Repo.get!(Shiftline.Coordination.CoverageRequest, request.id).status ==
               "claimed"
      ...
      approved = Shiftline.Repo.get!(Shiftline.Coordination.CoverageRequest, request.id)
  ...
  describe "shift tasks on Today" do
    test "the frontline view offers Claim on unowned work and not on other people's", %{
      ...
      [unassigned] =
        Shiftline.Repo.all(Shiftline.Coordination.ShiftTask)
        |> Enum.filter(&is_nil(&1.assignee_id))
    ...
    test "claiming work, starting it and finishing it", %{conn: conn, luis: luis} do
      ...
      claimed = Shiftline.Repo.get!(Shiftline.Coordination.ShiftTask, unassigned.id)
      ...
      assert Shiftline.Repo.get!(Shiftline.Coordination.ShiftTask, unassigned.id).status ==
               "in_progress"
      ...
      assert Shiftline.Repo.get!(Shiftline.Coordination.ShiftTask, unassigned.id).status == "done"
    end
    ...
    test "a supervisor can add work and hand it to someone", %{conn: conn, luis: luis} do
      ...
      task =
        Shiftline.Repo.get_by!(Shiftline.Coordination.ShiftTask, title: "Check the lobby lights")
      ...
      assert Shiftline.Repo.get!(Shiftline.Coordination.ShiftTask, task.id).assignee_id == luis.id
    end
  ...
  describe "settings" do
    test "changing language re-renders the whole surface in it", %{conn: conn} do
      ...
      assert html =~ "How you use Shiftline"
      ...
      assert html =~ "Votre utilisation de Shiftline"
    end
