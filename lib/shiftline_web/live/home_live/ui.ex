defmodule ShiftlineWeb.HomeLive.UI do
  @moduledoc """
  Shared function components and view helpers for the Shiftline prototype:
  ...
  def resolved_card(assigns) do
    assigns =
      assign(
        assigns,
        :gaps,
        Shiftline.Coordination.coverage_gaps(assigns.request, assigns.responses)
      )
  ...
  Reads the translation cache first — `Shiftline.Translation` fills it in the
  ...
  defp translation_of(text) do
    locale = Gettext.get_locale(ShiftlineWeb.Gettext)
    Shiftline.Translation.lookup(text, locale) || Gettext.gettext(ShiftlineWeb.Gettext, text)
  end
  ...
  def event_actor(%{actor: nil}), do: "Shiftline"
