defmodule Shiftline.Translation do
  @moduledoc """
  ...
  while a real deployment translates anything:
      ...
      config :shiftline, Shiftline.Translation, adapter: Shiftline.Translation.DeepSeek
      config :shiftline, Shiftline.Translation, adapter: Shiftline.Translation.Claude

  The two provider-backed adapters send an identical prompt
  (`Shiftline.Translation.Prompt`), so the choice between them is a cost and
  ...
  def adapter do
    :shiftline
    |> Application.get_env(Shiftline.Translation, [])
    |> Keyword.get(:adapter, Shiftline.Translation.Local)
  end
  ...
  def translate_later(text, on_complete) when is_binary(text) do
    cond do
      ...
      true ->
        Task.Supervisor.start_child(Shiftline.TaskSupervisor, fn ->
          translate_now(text)
          on_complete.()
        end)
    end
  ...
  defp async?, do: Application.get_env(:shiftline, Shiftline.Translation, [])[:async] != false
